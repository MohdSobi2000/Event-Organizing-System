...
** Note: Our design idea was to implement the facade design pattern in the ConferenceManagingSystem so that we would
have three smaller controllers (userSystem, eventSystem and messageSystem) that would each have a single responsibility
and we would instantiate them in ConferenceManagingSystem. We did not have time to create the UserSystem class, which
is why we had to instantiate the UserHandler inside the ConferenceManagingSystem. Our first step of phase 2 will be
to remove any interactions between ConferenceManagingSystem and UserHandler since it is a use case and the rest of
our ConferenceManagingSystem class is interacting only with controllers. We also chose to have the presenters return
strings which would be printed to the screen in the controllers.
